...
for i in range(N):
    gen.append(random.choice(np.round_(np.linspace(0,1,100), decimals=2)))
...
for j in range(1000):
    ...
    # 2. If the fitness is higher or equal than the selected fitness, pick 2 parents and do crossover
    # 3. Append the new x-value to the newGen array
    for i in gen:
        r = getSelectionIndex(prob)
        x = 0
        if(fitness(i) >= fitness(gen[r])):
            parent1 = gen[getSelectionIndex(prob)]
            parent2 = gen[getSelectionIndex(prob)]
            aValue = random.uniform(0,1)

            # CROSSOVER
            x = aValue*parent1 + (1-aValue)*parent2
            if(x > 1):
                x = 1
            elif(x < 0):
                x = 0
